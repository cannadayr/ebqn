...
call(F,X,W) ->
    ...
    F(X,W).
r(H,{R,I}) when is_reference(R) -> % resolve heap reference when popping from stack
    #e{s=E} = fetch(R,H),
    array:get(I,E);
r(_H,X) when is_function(X); % pass thru
          is_record(X,v);
          is_record(X,m1);
          is_record(X,m2);
          is_number(X);
...
ge(I,E,H) ->
    #e{p=P} = fetch(E,H),
    ge(I-1,P,H).
hset(H,D,#v{r=Id},#v{r=V}) ->
    foldl(fun(J,N,A) -> hset(A,D,N,array:get(J,V)) end,H,Id);
hset(H,D,{E,I},V) ->
    #e{s=A} = E0 = fetch(E,H),
    D = (array:get(I,A) =:= null),
    store(E,E0#e{s=set(I,r(H,V),A)},H).
...
se(_B,O,_D,_H,_RootEnv,_E0,S,X,0) ->
    cons(element(1+X,O),S);
se(_B,_O,_D,_H,_RootEnv,_E0,S,X,3) ->
    {T,Si} = case X of
        0 -> {list(fixed([])),S};
        _ -> tail(X-1,new(X),S)
    end,
    cons(list(T),Si);
se(_B,_O,_D,_H,_RootEnv,_E0,S,X,4) ->
    {T,Si} = case X of
        0 -> {list(fixed([])),S};
        _ -> tail(X-1,new(X),S)
    end,
    cons(list(T),Si);
se(_B,_O,_D,_H,_RootEnv,_E0,S,undefined,7) ->
    F = head(S),
    #m1{f=M} = head(tail(S)),
    cons(M(F),tail(tail(S)));
se(_B,_O,_D,_H,_RootEnv,_E0,S,undefined,8) ->
    F = head(S),
    #m2{f=M} = head(tail(S)),
    G = head(tail(tail(S))),
    cons(M(F,G),tail(tail(tail(S))));
se(_B,_O,_D,_H,_RootEnv,_E0,S,undefined,9) ->
    G = head(S),
    J = head(tail(S)),
    cons(fun(X,W) -> call(G,call(J,X,W),undefined) end,tail(tail(S)));
se(_B,_O,_D,_H,_RootEnv,_E0,S,undefined,11) ->
    tail(S);
se(_B,_O,_D,_H,_RootEnv,_E0,S,undefined,14) ->
    liat(S);
se(B,O,D,H,RootEnv,E0,S,X,15) ->
    F = element(1+X,D),
    cons(F(B,O,D,H,RootEnv,E0),S);
se(_B,_O,_D,_H,_RootEnv,_E0,S,undefined,16) ->
    F = head(S),
    X = head(tail(S)),
    cons(call(F,X,undefined),tail(tail(S)));
se(_B,_O,_D,_H,_RootEnv,_E0,S,undefined,17) ->
    W = head(S),
    F = head(tail(S)),
    X = head(tail(tail(S))),
    cons(call(F,X,W),tail(tail(tail(S))));
se(_B,_O,_D,_H,_RootEnv,_E0,S,undefined,19) ->
    F = head(S),
    G = head(tail(S)),
    J = head(tail(tail(S))),
    cons(tr3o(J,G,F),tail(tail(tail(S))));
se(_B,_O,_D,H,_RootEnv,E0,S,{X,Y},21) ->
    {_T,#e{s=V}} = ge(X,E0,H),
    Z = array:get(Y,V),
    true = (null =/= Z),
    cons(Z,S);
se(_B,_O,_D,H,_RootEnv,E0,S,{X,Y},22) ->
    {T,_} = ge(X,E0,H),
    cons({T,Y},S);
se(_B,_O,_D,_H,_RootEnv,_E0,S,undefined,25) ->
     S.
...
vm_switch(B,O,D,P,H,RootEnv,E,S,cont) ->
    ArgStart = P+1,
    {Op,ArgStart} = num(B,P),
    ...
    {Arg,ArgEnd} = pe(B,ArgStart,Op),
        ...
        %dbg({Op,P},{15,0}),
    Sn = se(B,O,D,H,RootEnv,E,S,Arg,Op),
        %fmt({se,len(Sn)},8),
    ...
    Ctrln = ce(S,Op),
        %fmt({ctrl,Ctrln},8),
    vm_switch(B,O,D,ArgEnd,Hn,RootEnv,E,Sn,Ctrln);
vm_switch(_B,_O,_D,_P,H,_RootEnv,_E,_S,Rtn) ->
    fmt(H),dbg(),
    Rtn.
vm(B,O,D,H,RootEnv,P,E) ->
    vm_switch(B,O,D,P,H,RootEnv,E,queue:new(),cont).

run_env(B,O,D,H0,RootEnv,E0,V,ST) ->
    fun (SV) ->
        E = make_ref(),
        H = store(E,#e{s=concat(SV,V),p=E0},H0),
        vm(B,O,D,H,RootEnv,ST,E)
    end.
load_block(T,I,ST,L) ->
    fun (B,O,D,H,RootEnv,E) ->
        fmt({block,{RootEnv,T,I,ST,L}}),
        %mem(),
        ...
        C = run_env(B,O,D,H,RootEnv,E,V0,ST),
        F = case T of
            0 -> fun(N) -> N(nil) end;
...
run(B,O,S) ->
    ...
    D = run_init(S),
    LoadBlock = element(1,D),
    LoadBlock(B,O,D,H,RootEnv,E).
