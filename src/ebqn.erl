...
hset(#h{d=H}=Heap,D,{E,I},V) ->
    #e{s=A} = E0 = fetch(E,H),
    D = (array:get(I,A) =:= null),
    Heap#h{d=store(E,E0#e{s=set(I,r(H,V),A)},H)}.
...
se(_B,O,_D,_H,_E0,S,X,0) ->
    cons(element(1+X,O),S);
se(_B,_O,_D,_H,_E0,S,X,3) ->
    {T,Si} = case X of
        0 -> {list(fixed([])),S};
        _ -> tail(X-1,new(X),S)
    end,
    cons(list(T),Si);
se(_B,_O,_D,_H,_E0,S,X,4) ->
    {T,Si} = case X of
        0 -> {list(fixed([])),S};
        _ -> tail(X-1,new(X),S)
    end,
    cons(list(T),Si);
se(_B,_O,_D,_H,_E0,S,undefined,7) ->
    F = head(S),
    #m1{f=M} = head(tail(S)),
    cons(M(F),tail(tail(S)));
se(_B,_O,_D,_H,_E0,S,undefined,8) ->
    F = head(S),
    #m2{f=M} = head(tail(S)),
    G = head(tail(tail(S))),
    cons(M(F,G),tail(tail(tail(S))));
se(_B,_O,_D,_H,_E0,S,undefined,9) ->
    G = head(S),
    J = head(tail(S)),
    cons(fun(X,W) -> call(G,call(J,X,W),undefined) end,tail(tail(S)));
se(_B,_O,_D,_H,_E0,S,undefined,11) ->
    tail(S);
se(_B,_O,_D,_H,_E0,S,undefined,14) ->
    liat(S);
se(B,O,D,H,E0,S,X,15) ->
    F = element(1+X,D),
    cons(F(B,O,D,H,E0),S);
se(_B,_O,_D,_H,_E0,S,undefined,16) ->
    F = head(S),
    X = head(tail(S)),
    cons(call(F,X,undefined),tail(tail(S)));
se(_B,_O,_D,_H,_E0,S,undefined,17) ->
    W = head(S),
    F = head(tail(S)),
    X = head(tail(tail(S))),
    cons(call(F,X,W),tail(tail(tail(S))));
se(_B,_O,_D,_H,_E0,S,undefined,19) ->
    F = head(S),
    G = head(tail(S)),
    J = head(tail(tail(S))),
    cons(tr3o(J,G,F),tail(tail(tail(S))));
se(_B,_O,_D,H,E0,S,{X,Y},21) ->
    {_T,#e{s=V}} = ge(X,E0,H),
    Z = array:get(Y,V),
    true = (null =/= Z),
    cons(Z,S);
se(_B,_O,_D,H,E0,S,{X,Y},22) ->
    {T,_} = ge(X,E0,H),
    cons({T,Y},S);
se(_B,_O,_D,_H,_E0,S,undefined,25) ->
     S.
...
vm_switch(B,O,D,P,H,E,S,cont) ->
    ...
    Ctrln = ce(S,Op),
        %fmt({ctrl,Ctrln},8),
    vm_switch(B,O,D,ArgEnd,mark(Hn,[]),E,Sn,Ctrln);
vm_switch(_B,_O,_D,_P,_H,_E,_S,Rtn) ->
    Rtn.
vm(B,O,D,H,P,E) ->
    vm_switch(B,O,D,P,H,E,queue:new(),cont).

% nil heap and nil root environment, create heap with empty dict.
run_env(B,O,D,nil,nil,V,ST) ->
    fun (SV) ->
        H = #h{d=dict:new()},
        vm(B,O,D,H,ST,nil)
    end;
% nil root environment, create root environment.
run_env(B,O,D,#h{d=H0} = Heap,nil,V,ST) ->
    fun (SV) ->
        E = make_ref(),
        H = Heap#h{r=E,d=store(E,#e{s=concat(SV,V),p=E},H0)},
        vm(B,O,D,H,ST,E)
    end;
% normal case
run_env(B,O,D,#h{d=H0} = Heap,E0,V,ST) ->
    fun (SV) ->
        E = make_ref(),
        H = Heap#h{d=store(E,#e{s=concat(SV,V),p=E0},H0)},
        vm(B,O,D,H,ST,E)
    end.
load_block(T,I,ST,L) ->
    fun (B,O,D,H,E) ->
...
run(B,O,S) ->
    D = run_init(S),
    Block = element(1,D),
    Block(B,O,D,nil,nil). % initiate block w/ nil heap and nil root environment, see run_env/7
